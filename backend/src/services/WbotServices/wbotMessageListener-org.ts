...
const verifyQueue = async (
  ...
  if (queues.length === 1 ) {
    ...
    if (greetingMessage && !ticket.isGroup && !chatbot) {
      const listMessage = {
        text: formatBody(`\u200e${greetingMessage}`, ticket.contact),
      };
