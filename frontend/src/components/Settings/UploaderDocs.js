...
const Uploader = () => {
  ...
  const handleFileChange = (event) => {
    ...
    if (selectedFile && selectedFile.type === "application/pdf") {
      ...
      toast.error("Use somente arquivos em formato PDF!");
    }
  };
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      const response = await api.post(`/settings/doc-upload?ref=${selectedOption}`, formData);
...
return (
  <>
    <Grid spacing={3} container>
      ...
      >
        <Tab label="Termos e Privacidade" />
      </Tabs>
      ...
      <form onSubmit={handleSubmit} className={classes.fullWidth}>
          ...
          <FormControl className={classes.selectContainer}>
            ...
            >
              <MenuItem value="termos">Termos de Uso</MenuItem>
        ...
        <Grid item xs={12} sm={12} md={12} style={{ display: 'flex' }}>
  			<FormControl className={classes.fullWidth}>
      			...
      			<input
                    ...
                    style={{ marginTop: 15, marginBottom: 15 }}
      			/>
      			{selectedFileName ? selectedFileName : 'Escolher documento PDF'}
    			</label>
  			</FormControl>
		</Grid>
