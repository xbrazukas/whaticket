...
export function PlansManagerGrid(props) {
    ...
    const renderKanban = (row) => {
        return row.useKanban === false ? "Não" : "Sim";
    };
                ...
                <TableBody>
                    {records.map((row) => (
                        <TableRow key={row.id}>
                            ...
                            <TableCell align="center">R$ {row.value ? row.value.toLocaleString('pt-br', {minimumFractionDigits: 2, useGrouping: true, maximumFractionDigits: 2, minimumIntegerDigits: 1}).replace(',', '.') : '00.00'}
</TableCell>
...
export default function PlansManager() {
    ...
    const handleSelect = (data) => {
        ...
        setRecord({
            ...
            value: data.value ? data.value.toLocaleString('pt-br', { minimumFractionDigits: 2 }).replace(',', '.') : '0',
            useWhatsapp,
            useFacebook,
            useInstagram,
            useCampaigns,
